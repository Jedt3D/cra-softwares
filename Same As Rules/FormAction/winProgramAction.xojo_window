...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function AddProgram(programName As String, faculty as String, eduyear As String) As Boolean
		  ...
		  row.Column("prog_name") = programName
		  row.Column("fac_id") = faculty
		  row.Column("eduyear") = EduYear
		  
		  App.mDB.InsertRecord("program", row)
...
#tag Events PushButton1
	#tag Event
		Sub Action()
		  ...
		  If AddProgram(tfProgramName.Text, popupCode(popFaculty.Text,1), popEduYear.Text) Then
		    winProgram.RefreshDataList
		    Self.Close
		  Else
...
#tag Events popFaculty
	#tag Event
		Sub Open()
		  ...
		  Dim sql As String
		  sql = "Select concat(fac_id, "" "", fac_name) as fac_id_name FROM faculty order by fac_id Asc"
