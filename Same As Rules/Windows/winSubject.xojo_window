...
Begin Window winSubject
   ...
   Begin Listbox DataList
      ...
      Index           =   -2147483648
      InitialParent   =   ""
      InitialValue    =   "รายการที่	รหัสวิชา	รหัสวิชา EN	ชื่อวิชา	หน่วยกิต	หมายเหตุ"
      Italic          =   False
      Left            =   20
      LockBottom      =   True
      LockedInPosition=   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub RefreshDataList()
		  ...
		  Dim sql As String
		  sql = "Select `cra_subject`.`subj_id` , `cra_subject`.`subj_id_th` , `cra_subject`.`subj_id_en` , `cra_subject`.`subj_name` , `cra_subject`.`credit` , `cra_subject`.`remark` FROM `cra_subject` ORDER BY `cra_subject`.`subj_id` DESC"
