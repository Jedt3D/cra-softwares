...
Begin Window winProgram
   ...
   Begin Listbox DataList
      ...
      Index           =   -2147483648
      InitialParent   =   ""
      InitialValue    =   "หลักสูตรที่	ชื่อหลักสูตร	ชื่อคณะ	ปีการศึกษา"
      Italic          =   False
      Left            =   20
      LockBottom      =   True
      LockedInPosition=   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub RefreshDataList()
		  ...
		  Dim sql As String
		  sql = "Select `program`.`prog_id` , `program`.`prog_name` , `faculty`.`fac_name`, `program`.`eduyear` FROM `program` Inner Join `faculty` ON `program`.`fac_id` = `faculty`.`fac_id` ORDER BY `program`.`prog_id` Asc"
