...
Begin Window winStudent
   ...
   Begin Listbox DataList
      ...
      Index           =   -2147483648
      InitialParent   =   ""
      InitialValue    =   "ชื่อ	นามสกุล	รหัสนักศึกษา รจภ	รหัสนักศึกษา มหิดล	คณะ"
      Italic          =   False
      Left            =   20
      LockBottom      =   False
      LockedInPosition=   False
...
#tag Events DataList
	#tag Event
		Sub Open()
		  ...
		  Dim sql As String
		  sql = "Select CONCAT( `student_registration`.`title_id` , `student_registration`.`firstname`) As firstname, `student_registration`.`lastname` , `student_registration`.`student_id` , `student_registration`.`student_id_mahidol` , `student_registration`.`program_id` FROM `student_registration` ORDER BY `student_registration`.`student_id` Asc"
		  ...
		  If data <> Nil Then
		    While Not data.EOF
		      ...
		      data.IdxField(5).StringValue.DefineEncoding(Encodings.UTF8) _
		      )
		      data.MoveNext
		    Wend
		    data.Close
